"""R9 runtime-only build. Legacy R2-R8 sources stay in Git history but are not shipped."""
...
OUT=ROOT/'web-dist'
CORE=('index.html','r9.css','src/clinic-rules.js','src/r9-game.js')
...
def main():
    ...
    for folder,manifest in [('characters','assets/ui/character-manifest.json'),('workspace','assets/workspace/manifest.json'),('chibi','assets/chibi/manifest.json'),('clinic','assets/clinic/manifest.json'),('service','assets/service/manifest.json')]:
        audit=ROOT/'qa/current'/folder; audit.mkdir(parents=True,exist_ok=True); shutil.copyfile(ROOT/manifest,audit/'asset-provenance.json')
    files=list(CORE)
    ...
    manifest={'source_commit':subprocess.check_output(['git','rev-parse','HEAD'],cwd=ROOT,text=True).strip(),'renderer':'dom-r9','release_status':'BUILD_VERIFIED_PENDING_DEPLOYMENT','art_status':'R9_CLEAN_UI','gameplay_version':'R9_CLEAN_CLINIC_FLOW','files':[]}
    ...
    print('R9 runtime allowlist PASS:',len(files),'files')
