"""Verify the deployed R10 runtime against the expected commit."""
...
def fetch(path):
    with urlopen(Request(urljoin(base,path),headers={'Cache-Control':'no-cache','User-Agent':'Clinic-Kitchen-R10-release-check'}),timeout=30) as r:
...
assert info['renderer']=='canvas-chibi-r10',info
assert info['gameplay_version']=='R10_KITCHEN_REBUILD',info
...
print('Public R10 HTTPS build verified:',args.expect_sha,len(info['files']),'runtime files')
