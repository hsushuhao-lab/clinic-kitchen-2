"""Verify the deployed R9 runtime against the expected commit."""
...
def fetch(path):
    with urlopen(Request(urljoin(base,path),headers={'Cache-Control':'no-cache','User-Agent':'Clinic-Kitchen-R9-release-check'}),timeout=30) as r:
...
for attempt in range(12):
    ...
    time.sleep(10)
assert info['renderer']=='dom-r9',info
assert info['gameplay_version']=='R9_CLEAN_CLINIC_FLOW',info
...
print('Public R9 HTTPS build verified:',args.expect_sha,len(info['files']),'runtime files')
