"""Verify an actual public deployment, never infer success from a pushed commit."""
...
def fetch(path):
    with urlopen(Request(urljoin(base,path),headers={'Cache-Control':'no-cache','User-Agent':'Clinic-Kitchen-release-check'}),timeout=30) as r:
...
for attempt in range(12):
    ...
    time.sleep(10)
assert info['renderer']=='canvas2d',info
...
print('Public HTTPS build verified:',args.expect_sha, len(info['files']),'runtime files')
