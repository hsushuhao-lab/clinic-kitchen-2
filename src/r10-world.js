/* R10 top-strip chibi renderer. Presentation only; gameplay lives in r10-game.js. */
(() => {
  'use strict';
  const canvas=document.getElementById('worldCanvas');
  const host=document.getElementById('worldStrip');
  const message=document.getElementById('worldMessage');
  if(!canvas||!host) throw new Error('R10 world host missing');
  ...
  const state={stage:'consult',x:positions.consult,targetX:positions.consult,moving:false,carry:false,direction:'east',doctor:'speed',workUntil:0};
  const waiters=[];
  ...
  ['room-back','desk','prep','stove','rice','speed','heat','strategy'].forEach(name=>{
    load(name,'assets/chibi/'+name+'.webp');
  });
  ...
  function drawBackground(){
    ...
    else {ctx.fillStyle='#e6e8dd';ctx.fillRect(0,0,width,height);}
    ctx.fillStyle='#f7f3e9aa';ctx.fillRect(0,height-38,width,38);
    ctx.strokeStyle='#91a39a';ctx.lineWidth=2;ctx.beginPath();ctx.moveTo(width*.08,height-40);ctx.lineTo(width*.92,height-40);ctx.stroke();
  }

  function drawStations(){
    Object.entries(positions).forEach(([id,xn])=>{
      const im=images[stationAssets[id]], [dw,dh]=stationSizes[id], x=width*xn, y=height-42;
      ctx.fillStyle=id===state.stage?'#e4c87555':'#ffffff00';
  ...
  function drawDoctor(){
    ...
    ctx.fillStyle='#2037462a';ctx.beginPath();ctx.ellipse(x,y,24,6,0,0,Math.PI*2);ctx.fill();
  ...
  async function goTo(stage,{carry=false,messageText=''}={}){
    if(!positions[stage])throw new Error('Unknown R10 world stage '+stage);
    ...
    message.textContent=stage==='consult'?'回到診間':stage==='prep'?'抵達備料檯':stage==='wok'?'抵達炒鍋': '抵達配餐檯';
  }
  function reset(){state.stage='consult';state.x=positions.consult;state.targetX=positions.consult;state.moving=false;state.carry=false;state.direction='east';state.workUntil=clock+.4;message.textContent='醫師待命';}
  window.CKR10World={goTo,reset,setCarry:v=>state.carry=!!v,setDoctor:id=>{if(images[id])state.doctor=id;},whenReady,snapshot:()=>({ready,stage:state.stage,x:state.x,targetX:state.targetX,moving:state.moving,carry:state.carry,direction:state.direction})};
})();
