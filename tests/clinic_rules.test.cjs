...
// Replaced by R6 Requirements 1 & 2: patient & desk merged to consult, fridge & table merged to prep.
test('R6 4 consolidated stations (consult, prep, wok, serve) run left to right',()=>{assert.deepEqual(stations.map(s=>s.id),['consult','prep','wok','serve']);for(let i=1;i<stations.length;i++)assert.ok(stations[i].x>stations[i-1].x);});
