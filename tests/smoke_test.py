...
for element in ['app','patientRail','ticketNumber','patientPortrait','patientComplaint','symptomList','prescriptionGrid','gameMain','worldStrip','worldCanvas','stagePanel','statusBar']:
...
for name in ('src/clinic-rules.js','src/r10-world.js','src/r10-game.js'):
    subprocess.run(['node','--check',str(ROOT/name)],check=True)
print(f'R10 Static QA PASS: {len(p.ids)} unique DOM IDs; {len(p.urls)} local resources; JavaScript syntax.')
