...
try:
 with sync_playwright() as pw:
  ...
  def setup():
   ...
   enter('patient');enter('desk');enter('fridge')
   ...
   for _ in keys:page.keyboard.press('e');page.wait_for_timeout(80)
   for _ in range(3):page.keyboard.press('e');page.wait_for_timeout(250)
   page.wait_for_function('window.cookedDish.isSimmered')
   return order
  # Native patient silhouettes plus exactly three real image-backed order chips.
  assert page.locator('#requestIcons img').count()==3
  ...
  page.set_viewport_size({'width':1366,'height':768});done('native patient bust and 3 image-backed preferences; desktop/mobile fit')
  ...
  page.locator('#sessionContinueBtn').click();page.locator('#world').focus();page.keyboard.press('f')
  ...
  go('rice');page.keyboard.press('1' if order['rice']=='半碗飯' else '2');pos=s()['playerPos'];page.keyboard.press('e')
  expect(page.locator('#clinicResult')).to_be_visible();expect(page.locator('.bonk-actor')).to_be_visible()
  assert page.evaluate('CKClinic.snapshot().result.quality')==100-penalty
  start=service()['reaction']['start'];points=page.evaluate('CKShift.snapshot().points')
  page.wait_for_timeout(650);assert service()['reaction']['frame']>=1;assert s()['actor']['animation']=='bonk'
  page.keyboard.down('d');page.wait_for_timeout(250);page.keyboard.up('d');assert s()['playerPos']==pos
  page.wait_for_timeout(1600);assert service()['reaction']['start']==start and service()['reaction']['frame']==3
  assert page.evaluate('CKShift.snapshot().points')==points
  done('one dissatisfied result causes impact/dazed/rub/recovery while world and score remain frozen')
  for w,h in [(1366,768),(375,667)]:
   ...
   assert page.locator('#clinicComparison .comparison-icon').count()==6
  ...
  order=setup();page.keyboard.press('f');assert service()['penalty']==0
  go('rice');page.keyboard.press('1' if order['rice']=='半碗飯' else '2');page.keyboard.press('e')
