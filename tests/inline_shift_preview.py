...
def inline_document(root:Path)->str:
 assets={p.relative_to(root).as_posix():'data:'+mimetypes.guess_type(p.name)[0]+';base64,'+base64.b64encode(p.read_bytes()).decode() for p in (root/'assets').rglob('*') if p.is_file() and p.suffix in ('.png','.webp','.jpg') and 'art_direction' not in p.parts}
